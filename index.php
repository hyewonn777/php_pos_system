<?php
require 'auth.php'; // checks login
require 'db.php';

// Helper for safe query counts
function getCount($conn, $sql, $field) {
    $res = $conn->query($sql);
...
$totalSales = getCount($conn, "SELECT IFNULL(SUM(total), 0) AS total FROM bills", "total");
$totalProducts = getCount($conn, "SELECT COUNT(*) AS cnt FROM stock", "cnt");
$pendingOrders = getCount($conn, "SELECT COUNT(*) AS cnt FROM orders WHERE status='Pending'", "cnt"); // fallback 0 if no table
$upcomingAppointments = getCount($conn, "SELECT COUNT(*) AS cnt FROM bookings WHERE event_date >= CURDATE()", "cnt");
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<div class="content">
    ...
    <h1>Dashboard</h1>
    <p>Welcome Admin! Here’s your system overview:</p>
    <div class="cards">
      <div class="card">💰 Total Sales: ₱<?php echo number_format($totalSales, 2); ?></div>
      <div class="card">📦 Total Products: <?php echo $totalProducts; ?></div>
      <div class="card">🛒 Pending Orders: <?php echo $pendingOrders; ?></div>
      <div class="card">📅 Upcoming Appointments: <?php echo $upcomingAppointments; ?></div>
    </div>
  </div>
